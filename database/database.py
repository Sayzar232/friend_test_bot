...
class Database:
    ...
    async def update_after_test_creation(self, user_id: int, data):
        async with self.pool.acquire() as connection:
            async with connection.transaction():
    ...
    async def get_users_for_weekly_reminders(self):
        async with self.pool.acquire() as connection:
    ...
    async def get_total_tests_passed(self):
        async with self.pool.acquire() as connection:
            count = await connection.fetchval("SELECT COUNT(*) FROM tests_results;")
            return count or 0

    async def get_last_hundred_users(self):
        async with self.pool.acquire() as connection:
    ...
    async def get_most_common_answers_per_question(self, top_n: int = 1, max_questions: int = 10):
        async with self.pool.acquire() as connection:
            rows = await connection.fetch(
                """
    ...
    async def get_top_tests_by_takers(self, limit: int = 5):
        async with self.pool.acquire() as connection:
            rows = await connection.fetch(
                """
